...
//  --------------------------------------------------------------------------
void
  fmq_server_apply_certificate(fmq_server_t *self, char *cert_path)
  {
    assert (self);
...
//  --------------------------------------------------------------------------
void
  fmq_server_configure_curve(fmq_server_t *self, char *domains, char *certs_path)
  {
    assert (self);
    zstr_sendm (self->pipe, "CONFIGURE CURVE");
    zstr_sendfm (self->pipe, "%s", domains);
    zstr_sendf (self->pipe, "%s", certs_path);
  }

//  --------------------------------------------------------------------------
//  Force apply in-memory configuration data
void
  fmq_server_apply_configuration (fmq_server_t *self)
  {
      zstr_send (self->pipe, "APPLY CONFIG");
  }
//  ---------------------------------------------------------------------
...
server_new (zctx_t *ctx, void *pipe)
{
    ...
    self->auth = zauth_new(self->ctx);
      zauth_set_verbose (self->auth, true);
...
server_control_message (server_t *self)
{
    ...
    if (streq (method, "SET CERTIFICATE")) {
        ...
        zcert_apply(self->certificate, self->router);
        zcert_print(self->certificate);
        free (cert_path);
    }
    else
    if (streq (method, "CONFIGURE CURVE")) {
        char *domains = zmsg_popstr (msg);
        char *certs_path = zmsg_popstr (msg);
        zauth_configure_curve (self->auth, domains, certs_path);
        zsocket_set_curve_server (self->router, true);
        free (domains); free (certs_path);
    }
    else
...
server_client_message (server_t *self)
{
    ...
    client->expires_at = zclock_time () + client->heartbeat * 2;
