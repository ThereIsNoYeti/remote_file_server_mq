...
void
fmq_client_connect (fmq_client_t *self, const char *endpoint, const char *server_cert_path)
{
    zstr_sendm (self->pipe, "CONNECT");
    zstr_sendm (self->pipe, endpoint);
    zstr_send  (self->pipe, server_cert_path);
}
...
//  --------------------------------------------------------------------------
void
  fmq_client_apply_certificate(fmq_client_t *self, char *cert_path)
  {
    assert (self);
    zstr_sendm (self->pipe, "SET CERTIFICATE");
    zstr_sendf (self->pipe, "%s", cert_path);
  }

//  --------------------------------------------------------------------------
//  Force apply in-memory configuration data
void
  fmq_client_apply_configuration (fmq_client_t *self)
  {
      zstr_sendm (self->pipe, "APPLY CONFIG");
  }
//  ---------------------------------------------------------------------
...
static server_t *
server_new (zctx_t *ctx, char *endpoint, char *server_cert_path, zcert_t *client_cert)
{
    server_t *self = (server_t *) zmalloc (sizeof (server_t));
    self->ctx = ctx;
    self->endpoint = strdup (endpoint);
    self->dealer = zsocket_new (self->ctx, ZMQ_DEALER);
    self->certificate = zcert_load(server_cert_path);
      zcert_apply (client_cert, self->dealer);
      zsocket_set_curve_serverkey(self->dealer, zcert_public_txt(self->certificate));
      self->certificate_path = strdup(server_cert_path);//For use on destroy/reload events
...
server_destroy (server_t **self_p)
{
    ...
    if (*self_p) {
        ...
        zcert_destroy(&self->certificate);
        free(self->certificate_path);
...
client_control_message (client_t *self)
{
    ...
    if (streq (method, "SET CERTIFICATE")) {
        char *cert_path = zmsg_popstr (msg);
        self->certificate = zcert_load(cert_path);
        free (cert_path);
    }
    else
    if (streq (method, "APPLY CONFIG")) {
        if (self->config){
            client_apply_config (self);
        }
        else {
            printf ("E: cannot config from memory!\n");
            self->config = zconfig_new ("root", NULL);
        }
    }
    else
    ...
    if (streq (method, "CONNECT")) {
        ...
        char *server_cert_path = zmsg_popstr (msg);
        if (self->nbr_servers < MAX_SERVERS) {
            server_t *server = server_new (self->ctx, endpoint, server_cert_path, self->certificate);
...
client_server_message (client_t *self, server_t *server)
{
    ...
    server->expires_at = zclock_time () + self->heartbeat * 2;
        ...
        if (retval == 0)
        {
            for (server_nbr = 0; server_nbr < self->nbr_servers; server_nbr++) {
                ...
                if((server->expires_at > 0) && (zclock_time() >= server->expires_at))
                {
                    ...
                    char *certificate_path = strdup(server->certificate_path);
                    server_destroy(&self->servers[server_nbr]);
                    self->servers[server_nbr] = server_new(self->ctx, sav_endpoint, certificate_path, self->certificate);
                    self->dirty = true;
